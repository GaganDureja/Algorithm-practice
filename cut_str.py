...
def cut_str(txt,k):
	lst = ['']
	for x in txt.split():		
		if (len(lst[-1])+len(x)+1)<=k:
			if len(lst[-1]):
				lst[-1]+=' '
			lst[-1]+= x
		else:
			lst.append(x)
	return lst if txt not in lst else 'null'
...
print(cut_str('the quick brown fox jumps over the lazy dog',10))
print(cut_str('aaaaaaaaaaaaaaaaaaaaaaaa',5))